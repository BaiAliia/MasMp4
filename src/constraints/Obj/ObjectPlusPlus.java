...
public class ObjectPlusPlus extends ObjectPlus implements Serializable {
    ...
    private Hashtable<String, HashMap<Object, ObjectPlusPlus>> links = new Hashtable<String, HashMap<Object, ObjectPlusPlus>>();

    /** Stores information about all parts connected with any objects. */
    private static HashSet<ObjectPlusPlus> allParts = new HashSet<>();
    ...
    public void showLinks(PrintStream stream) throws Exception {
        for(String obj : links.keySet()) {
            showLinks(obj, stream);
        }
    }
    ...
    public static void showAllLinks(Class roleName, PrintStream stream) throws Exception {
        Vector extent = null;

        if(extents.containsKey(roleName)) {
            extent = (Vector) extents.get(roleName);
        }
        else {
            throw new Exception("Unknown class " + roleName);
        }

        System.out.println("Class extention links " + roleName.getSimpleName());

        for(Object obj: extent) {
            stream.println("object: " + obj);
            ((ObjectPlusPlus) obj).showLinks(stream);
        }
    }
}
